...
export default function Home() {
  ...
  useEffect(() => {
    api.get('/testimoni')
      .then(res => setTestimoni(res.data.testimoni || []))
      .catch(() => {})
  }, [])

  useEffect(() => {
    api.get('/layanan')
      .then(res => setLayanan(res.data.layanan || []))
      .catch(() => {})
    ...
    <div style={{ fontFamily: "'Poppins', sans-serif", overflowX: 'hidden' }}>
      ...
      <section id="services" style={{ backgroundColor: '#0066FF', padding: '70px 0 80px' }}>
        <div style={{ ...W }}>
          ...
          {/* Data layanan SEPENUHNYA dari database (GET /api/layanan). Tidak ada fallback hardcode. */}
          {layananLoading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 20 }}>
              {[0, 1, 2].map(i => (
                <div key={i} style={{ backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 16, height: 260, animation: 'pulse 1.5s ease-in-out infinite' }} />
              ))}
            </div>
          ) : layanan.length === 0 ? (
            <div style={{ backgroundColor: '#ffffff', borderRadius: 16, padding: '48px 24px', textAlign: 'center' }}>
              <p style={{ color: '#555', fontWeight: 600, margin: 0 }}>Belum ada layanan yang ditampilkan saat ini.</p>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 20 }}>
              {layanan.map((item, i) => (
                <Reveal key={item.id} delay={i * 120}>
                  <div
                    style={{ backgroundColor: '#ffffff', borderRadius: 16, overflow: 'hidden', transition: 'transform 0.25s, box-shadow 0.25s' }}
                    onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 16px 40px rgba(0,0,0,0.2)' }}
                    onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none' }}
                  >
                    {item.gambar ? (
                      <img src={item.gambar} alt={item.judul} style={{ width: '100%', height: 180, objectFit: 'cover' }}
                        onError={e => { e.target.style.display = 'none' }} />
                    ) : (
                      <div style={{ width: '100%', height: 180, backgroundColor: '#EBF2FF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#0066FF" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.4 }}>
                          <rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>
                        </svg>
                      </div>
                    )}
                    <div style={{ padding: '16px 20px', textAlign: 'center' }}>
                      <p style={{ fontWeight: 600, color: '#111', fontSize: '0.95rem', marginBottom: item.deskripsi ? 6 : 10 }}>{item.judul}</p>
                      {item.deskripsi && (
                        <p style={{ color: '#777', fontSize: '0.82rem', lineHeight: 1.6, marginBottom: 10 }}>{item.deskripsi}</p>
                      )}
                      <button onClick={() => navigate('/register')}
                        style={{ border: '1.5px solid #0066FF', color: '#0066FF', backgroundColor: 'transparent', borderRadius: 20, fontSize: '0.8rem', padding: '5px 20px', cursor: 'pointer', fontFamily: "'Poppins', sans-serif", fontWeight: 500, transition: 'all 0.2s' }}
      ...
      <section style={{ backgroundColor: '#ffffff', padding: '80px 0' }}>
        <div style={{ ...W }}>
          ...
          {testimoni.length === 0 ? (
            /* Fallback kalau belum ada data di DB — tampilkan skeleton cards */
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 24, maxWidth: 860, margin: '0 auto' }}>
              {[0, 1].map(i => (
                <div key={i} style={{ backgroundColor: '#f3f4f6', borderRadius: 16, padding: '28px 24px', minHeight: 180, animation: 'pulse 1.5s ease-in-out infinite' }} />
              ))}
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 24, maxWidth: testimoni.length === 1 ? 420 : testimoni.length === 2 ? 860 : '100%', margin: '0 auto' }}>
              {testimoni.map((item, i) => (
                <Reveal key={item.id} delay={i * 120}>
                  <div
                    style={{ backgroundColor: '#0066FF', borderRadius: 16, padding: '28px 24px', transition: 'transform 0.25s', height: '100%', display: 'flex', flexDirection: 'column' }}
                    onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-6px)'}
                    onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}
                  >
                    <div style={{ fontSize: '2.5rem', color: '#FFD93D', lineHeight: 1, marginBottom: 12 }}>"</div>
                    <p style={{ color: '#fff', fontSize: '0.9rem', lineHeight: 1.8, marginBottom: 20, flex: 1 }}>{item.isi}</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      {item.foto ? (
                        ...
                        <div style={{ width: 40, height: 40, borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontWeight: 700, color: '#fff', fontSize: '1rem' }}>
                          {item.nama?.charAt(0).toUpperCase()}
                        </div>
                      )}
                      <div>
                        <p style={{ color: '#FFD93D', fontWeight: 700, fontSize: '0.9rem', margin: 0 }}>{item.nama}</p>
                        {item.role && <p style={{ color: '#cce0ff', fontSize: '0.8rem', margin: 0 }}>{item.role}</p>}
                      </div>
                    </div>
                  </div>
                </Reveal>
