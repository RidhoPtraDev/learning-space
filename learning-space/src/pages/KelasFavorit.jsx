...
export default function KelasFavorit() {
  ...
  const [favorites,  setFavorites]  = useState([])   // [{id, nama, icon, ...}] — data kelas
  const [allKelas,   setAllKelas]   = useState([])   // semua kelas untuk modal tambah
  ...
  const fetchFavorit = async () => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || 'Gagal memuat favorit')
    } finally {
      setLoading(false)
    }
  }
  ...
  // ── HAPUS DARI FAVORIT ──────────────────────────────────────
  const handleHapus = async (kelasId) => {
    try {
      await api.delete(`/favorit/${kelasId}`)
      setFavorites(prev => prev.filter(k => k.id !== kelasId))
    } catch (err) {
      alert(err.response?.data?.message || 'Gagal menghapus favorit')
    }
  }
  ...
  const handleTambahkan = async () => {
    ...
    setAdding(true)
    try {
      for (const kelasId of selectedToAdd) {
        await api.post('/favorit', { kelasId })
      }
      await fetchFavorit()
      setSelectedToAdd([])
      setModalSearch('')
      ...
      alert(err.response?.data?.message || 'Gagal menambahkan favorit')
    } finally {
      setAdding(false)
    }
  }

  const favoritIds  = favorites.map(k => k.id)
  const filteredFav = favorites.filter(k =>
    k.nama.toLowerCase().includes(search.toLowerCase())
  )
  const modalList   = allKelas.filter(k =>
    !favoritIds.includes(k.id) &&
    k.nama.toLowerCase().includes(modalSearch.toLowerCase())
  )
      ...
      <main style={s.main}>
        ...
        <div style={s.headerRow}>
          <div style={{ flex: 1 }}>
            ...
            <p style={s.pageDesc}>
              Kelas favorit adalah kumpulan kelas yang kamu simpan untuk diakses dengan lebih mudah kapan saja.
            </p>
          </div>
        ...
        {/* Loading */}
        {loading && (
          <div style={s.emptyState}><p style={{ color: '#888' }}>Memuat favorit...</p></div>
        )}
        ...
        {/* Empty */}
        {!loading && !errorMsg && filteredFav.length === 0 && (
        ...
        {!loading && !errorMsg && filteredFav.length > 0 && (
          <div style={s.grid}>
            {filteredFav.map(kelas => (
              <div
                key={kelas.id}
                style={s.card}
                onClick={() => navigate(`/kelas/${kelas.id}`)}
                onMouseEnter={e => { e.currentTarget.style.transform='translateY(-6px) scale(1.02)'; e.currentTarget.style.boxShadow='0 12px 32px rgba(0,102,255,0.45)' }}
                onMouseLeave={e => { e.currentTarget.style.transform='translateY(0) scale(1)'; e.currentTarget.style.boxShadow='0 4px 16px rgba(0,102,255,0.25)' }}
              >
                <button
                  style={s.bookmarkBtn}
                  onClick={e => { e.stopPropagation(); handleHapus(kelas.id) }}
                ...
                <p style={s.cardSub}>{kelas.kategori}</p>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={s.modalOverlay} onClick={() => setShowModal(false)}>
          <div style={s.modalBox} onClick={e => e.stopPropagation()}>
            ...
            <div style={s.modalList}>
              {modalList.length === 0 ? (
                <p style={{ textAlign: 'center', color: '#999', padding: '24px 0' }}>
                  Semua kelas sudah ditambahkan atau tidak ditemukan.
                </p>
              ) : (
                modalList.map(k => {
                  const checked = selectedToAdd.includes(k.id)
                  return (
                    <div key={k.id}
                      style={{ ...s.modalItem, backgroundColor: checked ? '#EBF2FF' : '#fff' }}
                      onClick={() => setSelectedToAdd(prev =>
                        prev.includes(k.id) ? prev.filter(x => x !== k.id) : [...prev, k.id]
                      )}
                    >
                      <img src={resolveIconSrc(k.icon)} alt={k.nama}
                        style={{ width: 40, height: 40, objectFit: 'contain', flexShrink: 0 }}
                        onError={e => e.target.style.display='none'}
                      />
                      <div style={{ flex: 1 }}>
                        <p style={s.modalItemTitle}>{k.nama}</p>
                        <p style={s.modalItemSub}>{k.kategori}</p>
                      </div>
            ...
            <div style={s.modalFooter}>
              <span style={{ fontSize: '0.85rem', color: '#666' }}>{selectedToAdd.length} dipilih</span>
              <button
                style={{ ...s.modalAddBtn, opacity: selectedToAdd.length === 0 || adding ? 0.5 : 1, cursor: selectedToAdd.length === 0 ? 'not-allowed' : 'pointer' }}
                onClick={handleTambahkan}
                disabled={selectedToAdd.length === 0 || adding}
              >
                {adding ? 'Menyimpan...' : 'Tambahkan ke Favorit'}
              </button>
            </div>
          </div>
        </div>
...
const s = {
  ...
  card: { backgroundColor: '#0066FF', borderRadius: '16px', padding: '28px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', cursor: 'pointer', position: 'relative', transition: 'transform 0.25s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.25s ease', boxShadow: '0 4px 16px rgba(0,102,255,0.25)', minHeight: '180px', justifyContent: 'center' },
  ...
  modalList: { flex: 1, overflowY: 'auto', padding: '0 24px', display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '8px' },
  ...
  modalAddBtn: { backgroundColor: '#0066FF', color: '#fff', border: 'none', borderRadius: '10px', padding: '11px 24px', fontWeight: 700, fontSize: '0.9rem', fontFamily: "'Poppins', sans-serif", cursor: 'pointer' },
}