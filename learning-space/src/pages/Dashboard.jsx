...
import ilustrasiDashboard from '../assets/ilustrasi-dashboard.png'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    api.get('/kelas')
      .then(res => setKelasList(res.data.kelas))
      .catch(err => setErrorMsg(err.response?.data?.message || 'Gagal memuat data kelas'))
  ...
  const toggleTemp    = k => setTempKategori(p => p.includes(k) ? p.filter(x => x !== k) : [...p, k])
  ...
  const filtered = kelasList.filter(k => {
    const matchSearch = k.nama.toLowerCase().includes(search.toLowerCase()) || (k.deskripsi || '').toLowerCase().includes(search.toLowerCase())
    ...
    <div style={s.layout}>
      ...
      <main style={s.main}>
        ...
        <div style={{ position: 'relative', marginBottom: 28 }}>
          ...
          {selectedKategori.length > 0 && <span style={s.filterBadge}>{selectedKategori.length}</span>}
          {showFilter && (
        ...
        {loading && <div style={{ textAlign: 'center', color: '#888', padding: '40px' }}>Memuat data kelas...</div>}
        {!loading && errorMsg && <div style={{ textAlign: 'center', color: '#dc2626', padding: '40px', backgroundColor: '#fee2e2', borderRadius: 16 }}>{errorMsg}</div>}
        {!loading && !errorMsg && (
          <div style={s.grid}>
            {filtered.map(kelas => (
              ...
              >
                <div style={s.cardLeft}>
                  ...
                  {(kelas.deskripsi || '').split('. ').filter(Boolean).map((line, i) => (
                    <p key={i} style={s.cardDesc}>{line.trim()}{line.endsWith('.') ? '' : '.'}</p>
                  ))}
                </div>
                <img src={resolveIconSrc(kelas.icon)} alt={kelas.nama} style={s.cardIcon}
                  onError={e => e.target.style.display = 'none'} />
              </div>
            ))}
            {filtered.length === 0 && (
